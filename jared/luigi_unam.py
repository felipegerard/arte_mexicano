...
class DirectorioLibros(luigi.ExternalTask):
    """
    Vamos a dar de alta el directorio con los libros
    """
    filename = luigi.Parameter()

    def output(self):
        """
        Solo va a regresar el directorio donde viven
        todos los libros
        """
...
class JuntarPDFs(luigi.Task):
    """
    Este proceso lista los pdfs de cada libro y junta
    todos los pdfs en pocos archivos 
    """
    cantidad_libros = luigi.IntParameter(default=0)
    warehouse = luigi.Parameter()
    carpeta_salida_libros = luigi.Parameter()

    def requires(self):
        return DirectorioLibros(self.warehouse)
    ...
    def run(self):
        ...
        def obtener_carpetas_base(carpeta_base,cantidad_libros):
            """
            Funcion para obtener las carpetas de los libros
            """
            #cantidad_libros = 500
        ...
        def obtener_contenido_libro(ruta_libro):
            """
            Deberia regresar un listado de los pdfs dentro del libro, 
            ordenados por el numero del nombre del archivo junto con el
            tamaño del archivo (bytes), si hay portada, al final del 
            listado esta en jpg
            """
        ...
        carpetas_base_libros = obtener_carpetas_base(self.warehouse,self.cantidad_libros)
        generar_libros(self.warehouse,carpetas_base_libros, self.carpeta_salida_libros, libros_no_agrupados)

    def output(self):
        return luigi.LocalTarget(self.carpeta_salida_libros)
