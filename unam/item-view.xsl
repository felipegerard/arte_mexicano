...
<xsl:stylesheet
	...
	<h2><i18n:text>xmlui.gil.similitud.titulo</i18n:text></h2>
	...
	<!-- #############################################################################################
		...
		MODIFICACIONES NECESARIAS PARA LEER ARCHIVO XML (archivoSimilitud.xml) CON DATOS DE SIMILITUD 
		
		##############################################################################################
		-->
         <!-- identificamos el handle del item que se estÃ¡ mostrando al usuario y le quitamos la parte que es compun a todos 'http://hdl.handle.net/'  -->
         <!-- el handle recortado lo podemos imprimir con la siguiente lÃ­nea -->
         <!--Handle_recortado: <xsl:value-of select="substring-after($context//dim:field[@element='identifier'],'http://hdl.handle.net/')"/> -->
         <xsl:variable name="handleRecortado" select="substring-after($context//dim:field[@element='identifier'],'http://hdl.handle.net/')"/>

         <!-- Leemos el contenido del archivo archivoSimilitud.xml y lo guardamos en la variable archivoSimilitud -->
        <xsl:variable name="archivoSimilitud" select="document('archivoSimilitud.xml')"/>
       <!-- <xsl:value-of select="$archivoSimilitud"/> -->
        
         

        <xsl:choose>
          <!-- recorremos el archivo de similitud para localizar la secciÃ³n que corresponde a los libros similares para el libro mostrado en pantalla, cuyo handle recortado tenemos guardado en la variable handleRecortado -->
          <xsl:when test="$archivoSimilitud//libro[@handle=$handleRecortado]/similar">   <!-- si el item tiene elementos similares en el $archivoSimilitud ... -->
             <!-- Leemos cada uno de los elementos similares a el libro mostrado en pantalla -->
            <xsl:for-each select="$archivoSimilitud//libro[@handle=$handleRecortado]/similar">
              <ul> 
              <!-- Imprimimos el hipervÃ­nculo <href> que se encuentra en archivoSimilitud usando como texto el <titulo> de el libro similar, este dato tambiÃ©n se encuantra en el archivoSimilitud -->
              <!-- <xsl:value-of select="href"/> esta instrucciÃ³n solamente trae el href sin formato de hipervÃ­nculo-->
                <li><a href="{href}"> <xsl:value-of select="titulo"/> </a> </li>
              </ul>
            </xsl:for-each>
          </xsl:when>

          <xsl:otherwise>  <!-- si no existen similares ... -->
            <i18n:text>xmlui.gil.similitud.vacio</i18n:text>
          </xsl:otherwise>
        </xsl:choose>
